...
if(!file.exists(paste0(outdir,'/clustering.rdata')) | !file.exists(paste0(outdir,'/centers.rdata')) ){
  ...
  # SPLINE FITTING --------------------------------------
snap_500 <- array(NA,c(Z_SIZE,Y_SIZE,X_SIZE)) 
snap_mips <- array(NA,c(Z_SIZE,Y_SIZE,X_SIZE)) 
active_mask <- array(NA,c(Z_SIZE,Y_SIZE,X_SIZE))
  ...
  for (ss in 1:Z_SIZE) {
	...
	for (ii in 1:Y_SIZE) {
        for (jj in 1:X_SIZE) {
          ...
          full_mat2[count,] <- detrend(full_mat2[count,], 'linear')
	    ...
	    active_mask[ss,ii, jj] <-  (max(full_mat2[count,],na.rm=T) - min(full_mat2[count,],na.rm=T)) > min_activity*sd(full_mat2[count,], na.rm=T)
	    snap_mips[ss,ii, jj] <- max(full_mat2[count,],na.rm=T)
	    snap_500[ss,ii,jj] <- full_mat2[count,250]
      	}
	}
      ...
      which_odds<-(1:length(file_number))%%2
	
	    print(dim(t(full_mat2)))
	
      FD <- smooth.basisPar(file_number[!!which_odds],t(full_mat2),Basis)
      ...
      pred_splines2$time<-1:(length(file_number))
      ...
      for(i in 1:20){
        ...
        plot<-ggplot()+geom_line(data=pred_splines2,aes(y=get(p),x=time), color='blue')+ geom_point(aes(x=2*(1:dim(full_mat2)[2]-1), y=full_mat2[random_splines[i],]))+scale_y_continuous("Intensity")
        pdf(paste0(outdir,'/individual_plots/point_', random_splines[i], '_slice_', ss+Z_START, '.pdf',sep=''),paper='a4r')
        print(plot)
        dev.off()
      }
      ...
      save(snap_500, file=paste(outdir,'/snap_500.rdata',sep=''))
      save(snap_mips, file=paste(outdir,'/snap_mips.rdata',sep=''))
    }

  }

  snap_500[is.na(snap_500)]<-0
  snap_mips[is.na(snap_mips)]<-0
  f.write.nifti(snap_500,file=paste0(outdir,'/snapshot.nii'), nii=TRUE)
  ...
  for (ss in 1:Z_SIZE) {
    load(paste(outdir,'/coeff_mat_',ss,'.rdata',sep=''))
    InCount <- 0
    for (ii in 1:Y_SIZE) {
      for (jj in 1:X_SIZE) {
        InCount <- InCount + 1
  ...
  #& (active_mask[ss,ii,jj] & !is.na(active_mask[ss,ii,jj])) 
  ...
  #   BIC
  # }
  if(file.exists(paste0(outdir,'/centers.rdata'))){load(file=paste0(outdir,'/centers.rdata'))}
  ...
  if(!exists('clustering')){
    ...
    if(start_kk <= max_clust){ #check if havent gone too far
      ...
      for (kk in (start_kk:max_clust)) {
        ...
        TIME <- proc.time()
        BIC_val[kk] <- cluster_BIC(big_mat,tkmeans(big_mat,kk,.9 ))
    ...
    load(paste0(outdir,'/centers.rdata'))
...
save(image_hold,file=paste(outdir,'/image_hold.rdata',sep=''))
image_hold[is.na(image_hold)]<-0
f.write.nifti(image_hold,file=paste0(outdir,'/clusters.nii'), nii=TRUE)
...
save(PRED,file=paste(outdir,'/predicted_means.rdata',sep=''))
...
for (ii in 1:dim(centers)[1]) {
  ...
  whom<-which(clustering_cluster==ii)
  n_samp<-min(100, length(whom))
  
  pred_to_plot <- matrix(NA,n_samp,dim(pred_range)[1])
  sample_to_plot<-big_mat[sample(which(clustering_cluster==ii),100),]
  for(j in 1:n_samp){
    pred_to_plot[j,] <- apply(pred_range,1,function(x) {sum(x*sample_to_plot[j,])})
  }
  
  pred_to_plot2<-data.frame(t(pred_to_plot))
  pred_to_plot2$time<-1:dim(pred_range)[1]
  pred_to_plot3<-melt(pred_to_plot2, id='time')
  
  pdf(paste0(outdir,'/cluster_timeseries_plots/cluster_', ii,'.pdf',sep=''),paper='a4r')
  ggplot(data=pred_to_plot3,aes(y=value,x=time,group=variable)) +geom_line(size=0.1)+scale_y_continuous("Intensity")+ggtitle(paste("Cluster ",ii ))
  dev.off()
...
save(CORR,file=paste(outdir,'/correlation_matrix.rdata',sep=''))
...
save(mean_image,file=paste(outdir,'/mean_image.rdata',sep=''))
mean_image[is.na(mean_image)]<-0
f.write.nifti(mean_image,file=paste0(outdir,'/mean_image.nii'), nii=TRUE)
...
for (cc in 1:comp) {
  pdf(paste(outdir,'/Cluster_Mean_Function_',cc,'.pdf',sep=''),paper='a4r')
...
pdf(paste(outdir,'/All_Cluster_Mean_Function.pdf',sep=''),paper='a4')
...
pdf(paste(outdir,'/Cluster_dendrogram.pdf',sep=''),paper='a4r')
plot(HCLUST)
dev.off()
...
pdf(paste0(outdir,'/Frequency_of_clusters.pdf',sep=''),paper='a4r')
