...
for(fold in folders){
  ...
  dir.create(paste0(outdir2, '/clusters/'), recursive=T)
  dir.create(paste0(outdir2, '/time_series/'), recursive=T)
  ...
  if(!file.exists(paste(outdir2,'/clusters/image_hold_merge.rdata',sep=''))){
    ...
    f.write.nifti(image_hold,file=paste0(outdir2,'/clusters/clusters_merge_orignal_numbering.nii'), nii=TRUE, L=header)
    ...
    for(g in 1:cl){ #need to skip background
      ...
      if((g-1)>0){
        f.write.nifti(temp_mat,file=paste0(outdir2,'/clusters/cluster_', g-1 ,'mask.nii'), nii=TRUE, L=header )
      }
    }

    # Plot Frequency Histogram
    pdf(paste0(outdir2,'/clusters/Frequency_of_clusters.pdf',sep=''),paper='a4r')
    plot(table(image_hold2)[-1],xlab='cluster',ylab='Frequency')
    dev.off()


    f.write.nifti(image_hold2,file=paste0(outdir2,'/clusters/clusters_merge.nii'), nii=TRUE, L=header)
    tab<-table(image_hold2)
    cl<-length(tab)
    print(tab)
    save(image_hold2,file=paste(outdir2,'/clusters/image_hold_merge.rdata',sep=''))
  ...
  ###########################################
  print(paste(outdir2,'/time_series/bg_data.rdata',sep=''))

  if(!file.exists(paste(outdir2,'/time_series/bg_data.rdata',sep=''))){
    if(!file.exists(paste(outdir2,'/time_series/temp_data.rdata',sep=''))){
      ...
      print("Reloading raw time series data")
      print(paste(outdir2,'/time_series/temp_data.rdata',sep=''))
      load(paste(outdir2,'/time_series/temp_data.rdata',sep=''))
    }

    if(k<Z_SIZE){
      for (s in k:Z_SIZE) {

        sink(file=paste(outdir2,'/time_series/prog.txt',sep=''))
        cat(s)
        cat("\n")
        sink()
        ...
        full_array <- list()
          ...
          rm(full_array)
          k<-s+1
          save(active_vox, count, count_bg, cluster_mat, cluster_counts, bg_mat, meta_mat, k, file= paste(outdir2,'/time_series/temp_data.rdata',sep=''))
    ...
    save(cluster_mat,file=paste(outdir2,'/time_series/main_data.rdata',sep=''))
    save(meta_mat,file=paste(outdir2,'/time_series/meta_data.rdata',sep=''))
    save(bg_mat,file=paste(outdir2,'/time_series/bg_data.rdata',sep=''))
  }else{
    load(file=paste(outdir2,'/time_series/main_data.rdata',sep=''))
    load(file=paste(outdir2,'/time_series/meta_data.rdata',sep=''))
    load(file=paste(outdir2,'/time_series/bg_data.rdata',sep=''))
  }
  ...
  if(cl>1){
    ...
    save(CORR,file=paste(outdir2,'/time_series/correlation_data.rdata',sep=''))
    pdf(paste(outdir2,'/Correlation_matrix.pdf',sep=''),paper='a4r')
  ...
  save(means2,file=paste(outdir2,'/time_series/time_series_plot_data.rdata',sep=''))

  samp_name<-basename(dirname(outdir2))


  # Plots
  plot<-ggplot(data=means2, aes(y=F0,x=Seconds, colour=Cluster))+geom_line()+theme_bw()

  pdf(paste(outdir2,'/', samp_name, 'Mean_time_series_by_cluster_.pdf',sep=''),paper='a4r')
  print(plot)
  dev.off()

  plot<-ggplot(data=means2, aes(y=F0,x=Seconds))+geom_line()+theme_bw()+facet_wrap(~Cluster)
  pdf(paste(outdir2,'/Mean_time_series_by_cluster_facet.pdf',sep=''),paper='a4r')
  print(plot)
  dev.off()
  ...
  for(i in 1:clust_n){
    ...
    pdf(paste(outdir2,'/', samp_name, 'Mean_time_cluster', levels(means2$Cluster)[i] ,'.pdf',sep=''),paper='a4r')
    print(plot)
    dev.off()
  }
  ...
  for(i in 1:clust_n){
    ...
    if(i !=clust_n){
      ...
      pdf(paste(outdir2,'/', samp_name, 'Mean_time_cluster', levels(means2$Cluster)[i] ,'_with_interval.pdf',sep=''),paper='a4r')
      print(plot)
      dev.off()
    }else{
  ...
  print(paste0("06-PostProc.R ", fold,   "  complete."))
}
###########################################
