list.of.packages <- c('snow','parallel', 'lattice', 'AnalyzeFMRI', 'ggplot2', 'reshape2', 'MASS', 'abind', 'fda', 'fields', 'speedglm','pracma', 'tclust', 'signal', 'capushe', 'pryr', 'lowmemtkmeans')
...
Completed_numbers<-Completed_numbers[!is.na(Completed_numbers)]
#print(Completed_numbers)
Incomplete_slices<-setdiff(1:Z_SIZE,as.numeric(Completed_numbers))
print(Incomplete_slices)

print(mem_used())
#rm(list = ls()[-c(which(ls()=="outdir"), which(ls()=="Incomplete_slices"))])




do_slice<-function(s, outdir){
  ...
  active_mask <- array(NA,c(X_SIZE,Y_SIZE))
  
  for (j in 1:Y_SIZE) {
    for (i in 1:X_SIZE) {
      count <- count + 1
      active_mask[i,j] <-  (max(mat_odds_filt[count,],na.rm=T) - min(mat_odds_filt[count,],na.rm=T))
  ...
  save(active_mask,file=paste(outdir,'/mips/MASK_active_', s, '.rdata',sep=''))
  ...
  while(i<21 & j<1000){
    ...
    if(active_mask[temp%%X_SIZE+1, temp%/%X_SIZE+1]){
...
max_cores <- min(detectCores()-1,30)


while(length(Incomplete_slices)>0){
  no_cores<-min(length(Incomplete_slices), max_cores)
  to_do<-Incomplete_slices[1:no_cores]
  print(paste("Doing Slices:" ,to_do))
  cl <- makeCluster(no_cores , type = "SOCK")
  clusterCall(cl, load_libs)
  output_tmp<-clusterApply(cl,to_do, do_slice, outdir);
  stopCluster(cl)
  print(paste("Rechecking output"))
  #recheck which complete
  Completed_output <- list.files(path=paste0(outdir, '/coeff_mats/'), pattern='coeff_mat', full.names=FALSE)
  Completed_numbers<-(as.numeric(unlist(strsplit(unlist(Completed_output), "[^0-9]+"))))
  Completed_numbers<-Completed_numbers[!is.na(Completed_numbers)]
  #print(Completed_numbers)
  Incomplete_slices<-setdiff(1:Z_SIZE,as.numeric(Completed_numbers))
  print(paste("There are",length(Incomplete_slices), "remaining."))
}
...
#do_slice(1)
#mcparallel(do_slice(k), silent = T)
