...
#probably put a better check here
  ...
  for (ss in 1:Z_SIZE) {
    ...
    if (paste('coeff_mat_',ss,'.rdata',sep='')%in%Completed_output & paste('coeff_mat_lspec_',ss,'.rdata',sep='')%in%Completed_output & paste('coeff_mat_spec_',ss,'.rdata',sep='')%in%Completed_output){
      ...
      print('freq domain splines')
      n_pred_time_points = 200
      pred_points<-seq(from = 0, to = (max_file-1)*Z_SIZE+Z_SIZE, length.out = n_pred_time_points)
      
      #get spline predictions
      spline_pred <-predict(FD, pred_points)
      print(dim( spline_pred))
      
      spline_freq <-spectrum(spline_pred[,1], spans = 3, plot=F)$freq
      spline_spectrum<-matrix(NA,length(spline_freq),prod(Y_SIZE*X_SIZE))
      
      for(i in 1:prod(Y_SIZE*X_SIZE)){
        spline_spectrum[,i] <- spectrum(spline_pred[,i], spans = 3, plot=F)$spec# is this a good smoothig choice?
      }  
      print(dim( spline_spectrum))
      ...
      FD_2 <- smooth.basisPar(pred_points_2, spline_spectrum, Basis_2)
      coeff_mat_spec <- as.matrix(t(FD_2$fd$coefs))
      
      #log of spectrum
      FD_3 <- smooth.basisPar(pred_points_2, log(spline_spectrum), Basis_2)
      ...
      save(coeff_mat,file=paste(outdir,'/coeff_mat_',ss,'.rdata',sep=''))
      save(coeff_mat_spec,file=paste(outdir,'/coeff_mat_spec_',ss,'.rdata',sep=''))
      save(coeff_mat_lspec,file=paste(outdir,'/coeff_mat_lspec_',ss,'.rdata',sep=''))
      }
    }
...
#     
 ...
 #      
  ...
  Count <- 0
  for (ss in 1:Z_SIZE) {
    print(paste(outdir,name,ss,'.rdata',sep=''))
    load(paste(outdir,name,ss,'.rdata',sep=''))
    if(name=='/coeff_mat_spec_'){coeff_mat <- coeff_mat_spec}
    if(name=='/coeff_mat_lspec_'){coeff_mat <- coeff_mat_lspec}
    InCount <- 0
    for (ii in 1:Y_SIZE) {
      for (jj in 1:X_SIZE) {
        InCount <- InCount + 1
  ...
  print(dim(big_mat))
  print(ssDM)### Scale the coeffient matrix for K-means
  ...
  save(mean_sd_from_unscaled, file = paste0(outdir,"/", name, "scalingparams.rdata"))
  print("Matrix successfully loaded and rescaled.")
  ...
  #   log_like_comps = m0ste0(indir,'/', name,'centers.rdata')ax_val+log(rowSums(exp(inner_minus)))
 ...
 # if(file.exists(paste0(indir,'/', name,'centers.rdata'))){load(file=paste0(indir,'/', name,'centers.rdata'))}
  ...
  if(!file.exists(paste0(outdir,'/', name,'centers.rdata'))){
    ...
    max_clust<-20 #speed up by making this look at smaller

    # load any partial results already saved
    if(file.exists(paste0(outdir,'/', name,'Time_store.rdata'))){load(file=paste0(outdir,'/', name,'Time_store.rdata'))}

    if(file.exists(paste0(outdir,'/', name,'BIC_values.rdata'))){load(file=paste0(outdir,'/', name,'BIC_values.rdata'))}
    ...
    if(start_kk <= max_clust){ #check if havent gone too far
      ...
      for (kk in (start_kk:max_clust)) {
        ...
        # Save the results
        save(TIME_STORE,file=paste0(outdir,'/', name,'Time_store.rdata'))
        save(BIC_val,file=paste0(outdir,'/', name,'BIC_values.rdata'))
      }
    }
    ### Get the optimal K and computer clustering under optimal K
    n <- dim(big_mat)[1]
    ...
    #save centres
    save(clustering,file=paste0(outdir,'/', name,'centers.rdata'))
  }else{
    print("Clustering already done. Loading saved data.")
    load(paste0(outdir,'/', name,'centers.rdata'))
  ...
  save(clustering_cluster,file=paste(outdir,'/', name,'clustering.rdata',sep=''))
...
#reload params
load(file = paste0(outdir,'/', name,'scalingparams.rdata'))
load(file=paste0(outdir,'/', name,'centers.rdata'))
load(file=paste0(outdir,'/', name,'clustering.rdata'))
...
clustering_cluster <- nearest_cluster(big_mat,clustering)
save(clustering_cluster, file=paste0(outdir,'/', name,'cluster_allocations.rdata'))
...
save(image_hold,file=paste(outdir,'/', name,'image_hold.rdata',sep=''))
image_hold[is.na(image_hold)]<-0
f.write.nifti(image_hold,file=paste0(outdir,'/', name,'clusters.nii'), nii=TRUE)
...
if(!exists("mean_sd_from_unscaled")){
  if(file.exists(paste0(outdir,'/', name,'scalingparams.rdata'))){
    load(paste0(outdir,'/', name,'scalingparams.rdata'))
  }
}
...
save(PRED,file=paste(outdir,'/', name,'predicted_means.rdata',sep=''))
...
print(" where am III" )
save(CORR,file=paste(outdir,'/', name,'correlation_matrix.rdata',sep=''))
...
save(mean_image,file=paste0(outdir,'/', name,'mean_image.rdata'))
mean_image[is.na(mean_image)]<-0
f.write.nifti(mean_image,file=paste0(outdir,'/', name,'mean_image.nii'), nii=TRUE)
...
for (cc in 1:comp) {
  pdf(paste0(outdir,'/', name,'Cluster_Mean_Function_',cc,'.pdf'),paper='a4r')
...
pdf(paste0(outdir,'/', name,'Correlation_matrix.pdf'),paper='a4r')
...
pdf(paste0(outdir,'/', name,'Cluster_dendrogram.pdf'),paper='a4r')
plot(HCLUST)
dev.off()
...
pdf(paste0(outdir,'/', name,'Frequency_of_clusters.pdf'),paper='a4r')
