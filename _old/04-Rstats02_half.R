...
if(!file.exists(file=paste(outdir,'/settings.rdata',sep=''))){
  save(X_START,Y_START,Z_START,X_SIZE,Y_SIZE,Z_SIZE,NAME,indir,outdir,FROM ,TO ,mask_fn ,min_activity,Basis_number,file=paste(outdir,'/settings.rdata',sep=''))
}else{
...
for (s in Z_START:(Z_SIZE+Z_START)) {
  for (j in Y_START:(Y_SIZE+Y_START)) {
    for (i in X_START:(X_SIZE+X_START))  {
      D_Mask[i-X_START, j-Y_START, s-Z_START] <- MASK[i, j, s]>=.99995
    }
  }
}
ssDM <- sum(D_Mask)
  ...
  for (s in 1:Z_SIZE) {
    ...
    if (paste('coeff_mat_',s,'.rdata',sep='')%in%Completed_output){
	  ...
	  for (j in 1:Y_SIZE) {
        ...
        full_mat_detr[,odds]<-mat_odds_detr
        ...
        for (j in 1:Y_SIZE) {
          for (i in 1:X_SIZE) {
            ...
            snap_mips[i,j] <- max(full_mat[count,],na.rm=T)
          }
        }
    ...
    pred_splines <- matrix(NA,X_SIZE*Y_SIZE,length(file_number))
      ...
      for(i in 1:20){
        ...
        plot<-ggplot()+ geom_point(aes(x=seq(from=0,to=0.2*(TO-FROM), length.out=(length(file_number))), y=full_mat_detr[random_splines[i],]))+geom_line(data=pred_splines2,aes(y=get(p),x=time), color='blue')+scale_y_continuous("Intensity", limits=ylimits1)+scale_x_continuous("Time (s)")
