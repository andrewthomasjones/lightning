...
if(!file.exists(paste0(outdir,'/clustering.rdata')) | !file.exists(paste0(outdir,'/centers.rdata')) ){
  ...
  for (ss in 1:Z_SIZE) {
	...
	for (ii in 1:Y_SIZE) {
        ...
        full_mat2[count,] <- full_mat2[ii,]-speedlm.fit(full_mat2[ii,],cbind(1,file_number))$coefficients[2]*file_number
        # full_mat[ii,] <- detrend(full_mat[ii,])
        # print(c('detrending',ss,ii))
	active_mask[ss,ii, jj] <-  (max(full_mat2[count,]) - min(full_mat2[count,]))>min_activity 
      }
}
      ...
      FD <- smooth.basisPar(file_number,t(full_mat2),Basis)
  ...
  for (ss in 1:Z_SIZE) {
    ...
    for (ii in 1:Y_SIZE) {
      for (jj in 1:X_SIZE) {
        InCount <- InCount + 1
        if(D_Mask[ss,ii,jj] & active_mask[ss,ii,jj]) {
          Count <- Count + 1
